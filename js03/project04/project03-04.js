...
for (let i = 0; i < reviewers.length; i++) {
   ...
   reviewCode += `
      <tr><th>${reviewTitles[i]}</th></tr>
      <tr><td>${reviewers[i]}</td></tr>
      <tr><td>${reviewDates[i]}</td></tr>
      <tr><td>${starImages(stars[i])}</td></tr>
      <tr><td>${reviews[i]}</td></tr>
   </table>
   `;

   // Insert the review table into the article
